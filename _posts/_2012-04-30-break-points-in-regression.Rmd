...
set.seed(123)
# 真实模型
x = sort(runif(100)); y = 2 + 1*x + 4*(x>.5) + 3*(x-.5)*(x>.5) + rnorm(100)
par(mar = c(4, 4, 0, 0), family = 'serif', mgp = c(2, 1, 0))
plot(x, y, pch = 20, col = 'darkgray')
# 斜率不同，截距限制
fit1 = lm(y ~ 1 + x + I((x-.5)*(x>.5)))
lines(x, fitted(fit1), lwd = 2, col = 2)
# 斜率不同，截距也不同
fit2 = lm(y ~ 1 + x + I(x>.5) + I((x-.5)*(x>.5)))
...
```{r loess-comparison}
fit3 = loess(y ~ x, span = .2)
fit4 = loess(y ~ 1 + x + I(x>.5) + I((x-.5)*(x>.5)), span = 1, degree = 1)
par(mar = c(4, 4, 0, 0), family = 'serif', mgp = c(2, 1, 0))
plot(x, y, pch = 20, col = 'darkgray')
...
qplot(x, y, group = x > .5) + geom_smooth() # 按0.5前后分组
```
